...
class DependenciesLayerConstruct(Construct):
    ...
    def __init__(
        ...
        construct_id: str,
        *,
        athenaeum: str | None = None,
        athenaeum_path: str | None = None,
        ...
        if athenaeum:
            ...
            athenaeum_install_cmd = (
                f"pip install --no-cache-dir 'athenaeum{athenaeum}' -t /asset-output/python"
            )
            # Use current directory as asset path (minimal context)
            asset_path: str = str(Path(__file__).parent)
        else:
            # Local dev: install from source
            athenaeum_install_cmd = (
                "pip install --no-cache-dir /asset-input -t /asset-output/python"
            )
            asset_path = athenaeum_path  # type: ignore[assignment]
